...
def svf_process_block_numba(frame, lp_arr, bp_arr, env_arr, base_cutoff, env_depth, q, samplerate):
    ...
    for i in range(n):
        ...
        if rect > env_arr[0]:
            ...
            env_arr[0] = r * env_arr[0] + (1.0 - r) * rect
        ...
        if q_clamped > 10.0:
            q_clamped = 10.0
        out[i] = lp_arr[0] * (1.0 + (q_clamped - 0.8) * 0.1)
    return out

@njit
def compressor_process_block_numba(frame, threshold_db, ratio, makeup):
    n = frame.shape[0]
    out = np.empty(n, dtype=np.float32)
...
def svf_process_block_py(frame, st, base_cutoff, env_depth, q):
    ...
    for i, x in enumerate(frame):
        env = envelope_follower_sample(x, st, attack=0.0003, release=0.02)
...
def compressor_process_block_py(frame, threshold_db, ratio, makeup):
    out = np.empty_like(frame)
    thr = 10.0 ** (threshold_db / 20.0)
    gr_db = 0.0
    for i, x in enumerate(frame):
...
def _process_callback(frames):
    ...
    if not EFFECTS_BYPASS.get("comp", True):
        ...
        makeup = EFFECT_PARAMS.get("comp_makeup", 1.0)
        if USE_NUMBA_KERNELS:
            out_l, gr_l = compressor_process_block_numba(out_l, float(thr), float(ratio), float(makeup))
            out_r, gr_r = compressor_process_block_numba(out_r, float(thr), float(ratio), float(makeup))
            LAST_COMP_GR_DB = max(float(gr_l), float(gr_r))
        else:
            out_l, gr_l = compressor_process_block_py(out_l, thr, ratio, makeup)
            out_r, gr_r = compressor_process_block_py(out_r, thr, ratio, makeup)
